...
// Add this function near the top of the file, after the existing variable declarations
function getBestLanguage() {
    var languages = ['en', 'de']; // Add more languages as needed
...
(function e(t, i) {
    ...
    var n = t.defaults = {
        ...
        text: {
            ...
            de: {
                ...
                toggleOutline: "Inhaltsverzeichnis/Lesezeichen umschalten",
                ...
                share: "Teilen",
                mailSubject: "Ich möchte dir dieses FlipBook zeigen",
    ...
    var W = function (e) {
        var settings = i.extend(true, {}, n, e);
        settings.language = getBestLanguage();
        return settings;
    };
    var initUI = function (e, bookController) {
        var a = "df-ui";
        var o = "df-ui-wrapper";
        var r = a + "-" + "btn";
        var s = z(bookController.target);
        var uiContainer = bookController.ui = i(h.div, {
            class: a
        });
        var u = bookController.options;
        uiContainer.dispose = function () {
            ...
            f.remove();
            v.remove();
            if (uiContainer.shareBox) {
                if (uiContainer.shareBox.dispose) uiContainer.shareBox.dispose();
                uiContainer.shareBox = null
            }
            document.removeEventListener("keyup", be, false);
            window.removeEventListener("click", I, false);
            uiContainer.update = null;
            bookController = null
        };
        var d = function (e) {
            if (isNaN(e)) e = bookController.target._activePage;
            else if (e < 1) e = 1;
            else if (e > bookController.target.pageCount) e = bookController.target.pageCount;
            return e
        };
        var f = uiContainer.next = i(h.div, {
            class: r + " " + a + "-next " + u.icons["next"],
            title: s ? u.text[u.language].previousPage : u.text[u.language].nextPage,
            html: "<span>" + u.text[u.language].nextPage + "</span>"
        }).on("click", function () {
            bookController.next()
        });
        var p = uiContainer.prev = i(h.div, {
            class: r + " " + a + "-prev " + u.icons["prev"],
            title: s ? u.text[u.language].nextPage : u.text[u.language].previousPage,
            html: "<span>" + u.text[u.language].previousPage + "</span>"
        }).on("click", function () {
            bookController.prev()
        });
        var g = i(h.div, {
            class: r + " " + a + "-play " + u.icons["play"],
            title: u.text[u.language].play,
            html: "<span>" + u.text[u.language].play + "</span>"
        }).on("click", function () {
            var e = i(this);
            bookController.setAutoPlay(!e.hasClass(u.icons["pause"]))
        });
        if (u.autoPlay == true) {
            uiContainer.play = g;
            bookController.setAutoPlay(u.autoPlayStart)
        }
        var v = i(h.div, {
            class: o + " " + a + "-zoom"
        });
        var m = uiContainer.zoomIn = i(h.div, {
            class: r + " " + a + "-zoomin " + u.icons["zoomin"],
            title: u.text[u.language].zoomIn,
            html: "<span>" + u.text[u.language].zoomIn + "</span>"
        }).on("click", function () {
            bookController.zoom(1);
            uiContainer.update();
            if (bookController.target.startPoint && bookController.target.pan) bookController.target.pan(bookController.target.startPoint)
        });
        var b = uiContainer.zoomOut = i(h.div, {
            class: r + " " + a + "-zoomout " + u.icons["zoomout"],
            title: u.text[u.language].zoomOut,
            html: "<span>" + u.text[u.language].zoomOut + "</span>"
        }).on("click", function () {
            bookController.zoom(-1);
            uiContainer.update();
            if (bookController.target.startPoint && bookController.target.pan) bookController.target.pan(bookController.target.startPoint)
        ...
        v.append(m).append(b);
        var w = uiContainer.pageNumber = i(h.div, {
            class: r + " " + a + "-page"
        }).on("change", function () {
            var e = parseInt(uiContainer.pageInput.val(), 10);
            e = d(e);
            bookController.gotoPage(e)
        }).on("keyup", function (e) {
            if (e.keyCode == 13) {
                var t = parseInt(uiContainer.pageInput.val(), 10);
                t = d(t);
                if (t !== d(bookController.target._activePage || bookController._activePage)) bookController.gotoPage(t)
            }
        });
        uiContainer.pageInput = i('<input id="df_book_page_number" type="text"/>').appendTo(w);
        uiContainer.pageLabel = i('<label for="df_book_page_number"/>').appendTo(w);
        var x = i(h.div, {
            class: o + " " + a + "-size"
        });
        var P = i(h.div, {
            class: r + " " + a + "-help " + u.icons["help"],
            title: u.text[u.language].toggleHelp,
            html: "<span>" + u.text[u.language].toggleHelp + "</span>"
        }).on("click", function () { });
        var y = uiContainer.sound = i(h.div, {
            class: r + " " + a + "-sound " + u.icons["sound"],
            title: u.text[u.language].toggleSound,
            html: "<span>" + u.text[u.language].toggleSound + "</span>"
        }).on("click", function () {
            u.soundEnable = !u.soundEnable;
            uiContainer.updateSound()
        });
        uiContainer.updateSound = function () {
        ...
        function C(e) {
            uiContainer.search.removeClass("df-active")
        }
        if (typeof u.source == "string" && u.search == true) {
            var L = uiContainer.search = i(h.div, {
        ...
        var E = uiContainer.more = i(h.div, {
        ...
        if (typeof u.source == "string" && u.enableDownload == true) {
            ...
            var R = uiContainer.download = i('<a download target="_blank" class="' + O + '"><span>' + u.text[u.language].downloadPDFFile + "</span></a>");
            R.attr("href", u.source).attr("title", u.text[u.language].downloadPDFFile)
        }
        var F = l.hasFullscreenEnabled();
        if (!F) {
            e.addClass("df-custom-fullscreen")
        }
        uiContainer.switchFullscreen = function () {
            var e = l.getFullscreenElement();
            var t = bookController.container[0];
            if (uiContainer.isFullscreen != true) {
                bookController.container.addClass("df-fullscreen");
                ...
                uiContainer.isFullscreen = true
            } else {
                bookController.container.removeClass("df-fullscreen");
                uiContainer.isFullscreen = false;
        ...
        var M = uiContainer.fullScreen = i(h.div, {
            class: r + " " + a + "-fullscreen " + u.icons["fullscreen"],
            title: u.text[u.language].toggleFullscreen,
            html: "<span>" + u.text[u.language].toggleFullscreen + "</span>"
        }).on("click", uiContainer.switchFullscreen);
        var D = uiContainer.fit = i(h.div, {
        ...
        var A = uiContainer.shareBox = new t.Share(e, u);
        var j = uiContainer.share = i(h.div, {
            class: r + " " + a + "-share " + u.icons["share"],
            title: u.text[u.language].share,
            html: "<span>" + u.text[u.language].share + "</span>"
        }).on("click", function (e) {
            if (uiContainer.shareBox.isOpen == true) uiContainer.shareBox.close();
            else {
                uiContainer.shareBox.update(bookController.getURLHash());
                uiContainer.shareBox.show()
            }
        });
        var _ = uiContainer.startPage = i(h.div, {
            class: r + " " + a + "-start " + (s ? u.icons["end"] : u.icons["start"]),
            title: u.text[u.language].gotoFirstPage,
            html: "<span>" + u.text[u.language].gotoFirstPage + "</span>"
        }).on("click", function () {
            bookController.start()
        });
        var H = uiContainer.endPage = i(h.div, {
            class: r + " " + a + "-end " + (s ? u.icons["start"] : u.icons["end"]),
            title: u.text[u.language].gotoLastPage,
            html: "<span>" + u.text[u.language].gotoLastPage + "</span>"
        }).on("click", function () {
            bookController.end()
        });
        var W = uiContainer.pageMode = i(h.div, {
            class: r + " " + a + "-pagemode " + u.icons["singlepage"],
            html: "<span>" + u.text[u.language].singlePageMode + "</span>"
        }).on("click", function () {
            var e = i(this);
            bookController.setPageMode(!e.hasClass(u.icons["doublepage"]))
        });
        bookController.setPageMode(bookController.target.pageMode == t.PAGE_MODE.SINGLE);
        var V = uiContainer.altPrev = i(h.div, {
        ...
        var G = uiContainer.altNext = i(h.div, {
        ...
        var q = uiContainer.thumbnail = i(h.div, {
            class: r + " " + a + "-thumbnail " + u.icons["thumbnail"],
            title: u.text[u.language].toggleThumbnails,
            html: "<span>" + u.text[u.language].toggleThumbnails + "</span>"
        }).on("click", function () {
            var e = i(this);
            if (bookController.target.thumbContainer) {
                var t = bookController.target.thumbContainer;
                t.toggleClass("df-sidemenu-visible");
                e.toggleClass("df-active")
            } else {
                bookController.contentProvider.initThumbs();
                e.toggleClass("df-active")
            }
            if (e.hasClass("df-active")) {
                e.siblings(".df-active").trigger("click")
            }
            uiContainer.update(true)
        });
        var Z = uiContainer.outline = i(h.div, {
            class: r + " " + a + "-outline " + u.icons["outline"],
            title: u.text[u.language].toggleOutline,
            html: "<span>" + u.text[u.language].toggleOutline + "</span>"
        }).on("click", function () {
            var e = i(this);
            if (bookController.target.outlineContainer) {
                var t = bookController.target.outlineContainer;
                e.toggleClass("df-active");
                t.toggleClass("df-sidemenu-visible");
                if (e.hasClass("df-active")) {
                    e.siblings(".df-active").trigger("click")
        ...
        var allControlsList = u.allControls.replace(/ /g, "").split(","),
            moreControlsString = "," + u.moreControls.replace(/ /g, "") + ",",
            hiddenControlsString = "," + u.hideControls.replace(/ /g, "") + ",";
        if (U && B) {
            hiddenControlsString += ",fullScreen,"
        }
        var moreControlsList = moreControlsString.split(",");
        for (var $ = 0; $ < allControlsList.length; $++) {
            var J = allControlsList[$];
            if (hiddenControlsString.indexOf("," + J + ",") < 0) {
                var ee = uiContainer[J];
                if (ee != null && typeof ee == "object") {
                    if (moreControlsString.indexOf("," + J + ",") > -1 && J !== "more" && J !== "pageNumber") {
                        T.append(ee)
                    } else {
                        N.append(ee)
                    }
        ...
        function be(e) {
            switch (e.keyCode) {
                case me:
                    if (uiContainer.isFullscreen == true) {
                        uiContainer.fullScreen.trigger("click")
        ...
        uiContainer.update = function (i) {
            k("ui update");
            var a = bookController.target;
            var o = d(a._activePage || bookController._activePage);
            var r = a.pageCount || bookController.pageCount;
            var s = a.direction == t.DIRECTION.RTL,
                l = o == 1 || o == 0,
                u = o == r;
            uiContainer.next.show();
            uiContainer.prev.show();
            uiContainer.altNext.removeClass("disabled");
            uiContainer.altPrev.removeClass("disabled");
            if (l && !s || u && s) {
                uiContainer.prev.hide();
                uiContainer.altPrev.addClass("disabled")
            }
            if (u && !s || l && s) {
                uiContainer.next.hide();
                uiContainer.altNext.addClass("disabled")
            }
            uiContainer.pageInput.val(o);
            uiContainer.pageLabel.html(o + "/" + r);
            if (e.find(".df-sidemenu-visible").length > 0) {
                e.addClass("df-sidemenu-open")
            } else {
                e.removeClass("df-sidemenu-open")
            }
            if (i == true) bookController.resize();
            if (a.contentProvider.zoomScale == a.contentProvider.maxZoom) {
                uiContainer.zoomIn.addClass("disabled")
            } else {
                uiContainer.zoomIn.removeClass("disabled")
            }
            if (a.contentProvider.zoomScale == 1) {
                uiContainer.zoomOut.addClass("disabled")
            } else {
                uiContainer.zoomOut.removeClass("disabled")
            }
        };
        if (bookController.target != null) {
            bookController.target.ui = uiContainer
        }
        if (u.onCreateUI != null) u.onCreateUI(bookController)
    };
    var G = null;
        ...
        n.prototype = {
            ...
            setPageMode: function (e) {
                if (e == true) {
                    ...
                    this.ui.pageMode.html("<span>" + this.options.text[this.options.language].doublePageMode + "</span>");
                    this.ui.pageMode.attr("title", this.options.text[this.options.language].doublePageMode);
                    this.target.pageMode = t.PAGE_MODE.SINGLE
                } else {
                    this.ui.pageMode.removeClass(this.options.icons["doublepage"]);
                    this.ui.pageMode.html("<span>" + this.options.text[this.options.language].singlePageMode + "</span>");
                    this.ui.pageMode.attr("title", this.options.text[this.options.language].singlePageMode);
            ...
            setAutoPlay: function (e) {
                if (this.options.autoPlay) {
                    e = e == true;
                    var t = e ? this.options.text[this.options.language].pause : this.options.text[this.options.language].play;
            ...
            if (Array === s.contentSource.constructor || Array.isArray(s.contentSource) || s.contentSource instanceof Array) {
                            ...
                            xhrFields: {
                                onprogress: function (e) {
                                    if (e.lengthComputable) {
                                        ...
                                        s.updateInfo(s.options.text[s.options.language].loading + " PDF " + t.toString().split(".")[0] + "% ...")
                                    }
                                }
                            },
                            success: function (e) {
                    ...
                    if (s) {
                        ...
                        o.onProgress = function e(t) {
                            if (s) {
                                ...
                                if (isNaN(i)) {
                                    if (t && t.loaded) {
                                        s.updateInfo(s.options.text[s.options.language].loading + " PDF " + (Math.ceil(t.loaded / 1e4) / 100).toString() + "MB ...")
                                    } else {
                                        s.updateInfo(s.options.text[s.options.language].loading + " PDF ...")
                                    }
                                } else {
                                    s.updateInfo(s.options.text[s.options.language].loading + " PDF " + i.toString().split(".")[0] + "% ...")
                                }
                            }
                        }
                    }
                ...
                var h = function () {
                    if (s) {
                        ...
                        s.updateInfo(s.options.text[s.options.language].loading + " PDF Worker ...");
                        var e = document.createElement("a");
                        e.href = n.pdfjsWorkerSrc;
                        if (e.hostname !== window.location.hostname) {
                            s.updateInfo(s.options.text[s.options.language].loading + " PDF Worker CORS ...");
                            i.ajax({
                ...
                if (window.pdfjsLib == null) {
                    if (s) {
                        s.updateInfo(s.options.text[s.options.language].loading + " PDF Service ...");
                        M(n.pdfjsSrc + "?ver=" + t.version, function () {
                            if (typeof define === "function" && define.amd && window.requirejs) {
                                s.updateInfo(s.options.text[s.options.language].loading + " PDF Service (require) ...");
                                require.config({
                                    paths: {
    ...
    var Q = function (e) {
        ...
        function a(n, a, o) {
            ...
            r.container.info = i(h.div, {
                class: "loading-info"
            }).appendTo(r.container).html(r.options.text[r.options.language].loading + "...");
...
DFLIP.Share = function e(t) {
    function i(e, i) {
        ...
        n.box = t('<div class="df-share-box">').on("click", function (e) {
            e.preventDefault();
            e.stopPropagation()
        }).appendTo(n.wrapper).html('<span class="df-share-title">' + i.text[i.language].share + "</span>");
        ...
        n.mail = t("<a>", {
            ...
            href: "mailto:?subject=" + i.text[i.language].mailSubject + "&body=" + i.text[i.language].mailBody.replace("{{url}}", encodeURIComponent(n.shareUrl)),
            target: "_blank"
        }).on("click", function (e) {
            t(this).attr("href", "mailto:?subject=" + i.text[i.language].mailSubject + "&body=" + i.text[i.language].mailBody.replace("{{url}}", encodeURIComponent(n.shareUrl)));
            e.stopPropagation()
        });
